...
1. **Package recency** (JS-family starters only) — when `cmd_template` invokes a package-distributed CLI (`pnpm dlx create-next-app`, `pnpm dlx create-astro`, `pnpm dlx create-vite`, etc.), the published version date is a cheap proxy for "the CLI you are about to run was updated recently".
...
```bash
# Resolve the package name from cmd_template (e.g., `pnpm dlx create-next-app` -> `create-next-app`).
# Then:
pnpm view <package> version
pnpm view <package> time.modified
```

Then run the GitHub check below.

If the package name cannot be derived from `cmd_template` (e.g., `cmd_template` starts with `git clone` — the `10x-astro-starter` case), skip the package-registry step and run only the GitHub check.
...
Compare the most recent timestamp (`pnpm view ... time.modified` for JS, otherwise GitHub `pushed_at`) against today:
...
- Network call fails (offline, gh not authenticated, registry unreachable) — log the error, print "recency check unavailable: <reason>", proceed.
